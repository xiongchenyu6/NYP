...
public class SampleActivity extends Activity {
	...
	private static String url_all_user = "http://192.168.1.17/chenyu/db_readalllandlord.php";
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    try {
			...
			String place = null;
			  for (int i=0; i<array.length(); i++){
		        ...
		        mMap.addMarker(new MarkerOptions().position(temp).title(place)).showInfoWindow();
	    ...
	    mMap.setOnMarkerClickListener(new OnMarkerClickListener() {
	        public boolean onMarkerClick(Marker arg0) {
	        	Intent appInfo = new Intent(SampleActivity.this, ComfirmActivity.class);		
			    Bundle b =new Bundle();
			    b.putString("place",  String.valueOf(arg0.getTitle()));							    
			    appInfo.putExtras(b);				     
	...
	public void control(View v) throws JSONException{
		StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
		StrictMode.setThreadPolicy(policy); 
	    String uri = null; 
	    try{
	    ...
	    List<NameValuePair> params = new ArrayList<NameValuePair>();
	    JSONParser jsonParser = new JSONParser();
	    String json =  jsonParser.makeHttpReques(uri, "POST", params);

	    JSONObject jSONObject = new JSONObject(json);
		if(jSONObject.getInt("success")==0)
		{
			AlertDialog.Builder builder  =new AlertDialog.Builder(this); 
			builder.setPositiveButton("You didn't not make book yet", null).show();
			
		}
		else
		{JSONArray array = jSONObject.getJSONArray("users");
			  if(array.getJSONObject(0).getInt("landlord_C")==0)
			  {
					AlertDialog.Builder builder  =new AlertDialog.Builder(this); 
					builder.setPositiveButton("Please wait,your rent have not be comfirmed", null).show();
